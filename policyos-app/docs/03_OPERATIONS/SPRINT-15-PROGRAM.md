...
| CP9 Exact Query Locator and State-Revision Read Contracts | Implemented, validated, pending review | Adds closed query-only state/result/audit locators, mandatory expected revisions, and an exact state-revision read result exposing only the stored digest; repository implementation and concrete integration remain blocked. |
