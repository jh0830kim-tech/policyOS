...
async def _persist_runtime_atomic_write_set(
    ...
    async def commit(self, write_set: RuntimeAtomicWriteSet) -> RuntimeTransactionReceipt:
        ...
        try:
            async with self._session.begin():
                receipt = await _persist_runtime_atomic_write_set(
                    self._session, write_set, stored_at=reading.observed_at
                )
