...
from app.ai.production import AIOfficeProductionComposition
...
def create_artifacts_router(production: AIOfficeProductionComposition) -> APIRouter:
    """Create thin routes bound to one immutable production composition."""
    ...
    async def create_work_package(
        ...
        try:
            async with production.request_execution_scope_factory.open(
                ProviderAuditRepository(db)
            ) as composition:
                if (
                    composition.provider != production.blueprint.provider
                    or composition.model_id != production.blueprint.logical_model_id
                ):
                    raise OfficeExecutionError(
                        "configuration_error", "AI provider is not configured", 503
                    )
                return await OfficeApplicationService(db, composition).execute_work_package(
                    payload,
...
__all__ = ("create_artifacts_router",)
